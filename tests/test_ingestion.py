...
import pytest
from ingestion.sources import build_sources, FileFormat, SchemaVersion, TLCSource
...
def test_2015_is_csv_gps_schema():
    ...
    s = sources[0]
    assert s.file_format == FileFormat.CSV
...
def test_csv_url_format():
    sources = build_sources(["2015-01"])
    url = sources[0].url
    assert "s3a://" in url
    assert "yellow_tripdata_2015-01.csv" in url
